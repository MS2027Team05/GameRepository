...
-> 検証完了。TitleSceneにNetworkManager(DontDestroyOnLoad)を配置し、LobbyScene -> GameScene -> LobbySceneの同期遷移を整備済み。

> [!NOTE]
> **【端末構成とマルチプレイ仕様の確定事項】**
> - **本番構成**: 専用ホストPC 1台（サーバー 兼 観戦・Display 2マルチスクリーン投影） ＋ クライアントノートPC 4台（各プレイヤー端末: 2vs2）。
> - **デバッグ構成**: エディタ側で「Host as Player（プレイヤー兼任）」を選択することで、ホスト自身もキャラを操作して1台〜2台で素早くテスト可能。
> - **各モジュールへの影響**:
>   各ノートPCには `IsOwner == true` となる自キャラが1体のみスポーンするため、当初懸念されていた「1台での画面分割」や「入力デバイスの奪い合い」は発生しません。
>   したがって、**本ドキュメントに記載されている各担当モジュール（担当A: 物理、担当B: エイム、担当C: ボール、担当D: 判定）のコード設計は変更なく、そのままの仕様で実装可能**です。
